   ...
   */
	    ...
	    //Maybe read in the input?
	        parse:
                while (matcher.find()) {
            	    ...
            	    ArrayList locations = (ArrayList) map.get(input);
                	if (locations != null) {
                		locations.add(counter);
                	} else {
                		ArrayList newList = new ArrayList();
            	    	newList.add(counter);
            	        map.put(input, newList);
            	    }
                	counter++;
                }
            ArrayList targetLocations = (ArrayList) map.get(targetGram);
            if (targetLocations != null) {
                ...
                while (iter.hasNext()) { 
                    ...
                    if (!key.equals(targetGram)) {
                	    for (int i = 0; i < val.size(); i++) {
                	    	double min = Double.POSITIVE_INFINITY;
                	    	for (int j = 0; j < targetLocations.size(); j++) {
                	    		distance = Math.abs((Integer)val.get(i) - (Integer)targetLocations.get(j));
                	    		min = Math.min(distance, min);
                	    	}
                	    	context.write(new Text(key.toString()), new Text("" + func.f(min) + " " + "1.0"));
                	    }
                    }
                }
            } else {
                ...
                while (iter.hasNext()) { 
                    ...
                    ArrayList val = (ArrayList) entry.getValue(); 
                    for (int i = 0; i < val.size(); i++) {
                    	context.write(new Text(key.toString()), new Text("0.0" + " " + "1.0"));
                    }
                }
            }
        }
    ...
    public static class Combine1 extends Reducer<Text, Text, Text, Text> {
      ...
      public void reduce(Text key, Iterable<Text> values,
    	  ...
    	  for (Text val: values) {
    		  ...
    		  counter += Double.parseDouble(str[1]);
    		  sum += Double.parseDouble(str[0]);
    	  }
    	  context.write(new Text(key), new Text("" + sum + " " + counter));
      }
    }
    ...
    public static class Reduce1 extends Reducer<Text, Text, DoubleWritable, Text> {
        ...
        public void reduce(Text key, Iterable<Text> values,
            ...
            if (sum == 0) {
            	context.write(new DoubleWritable(sum), new Text(key));
            } else {
            	double rate = sum*Math.pow(Math.log10(sum), 3)/counter;
            	context.write(new DoubleWritable(rate), new Text(key));	
            }
        }
    }
    ...
    public static void main(String[] rawArgs) throws Exception {
        ...
        boolean runJob2 = conf.getBoolean("runJob2", false);
